"""IK retargeting engine.

Derived from General Motion Retargeting (GMR), Copyright 2025 Yanjieze,
MIT licensed — see NOTICE. Asimov-specific behaviour lives in the two-pass
match tables and the scale-ratio normalization.
"""
import mink
import mujoco as mj
import numpy as np
...
class GeneralMotionRetargeting:
    ...
    def __init__(
        ...
        # Normalize the subject onto the FIXED-size robot. The human joints come
        # from the body model posed with the clip's own betas, so they are already
        # subject-true-size (proportional to actual_human_height); for the scaled
        # targets to be subject-INDEPENDENT the table must be scaled by
        # assumption/actual — a short subject scales UP toward the reference size,
        # a tall one down. The previous actual/assumption direction compounded
        # subject size instead (targets ~ height^2): a 1.43 m subject produced a
        # 0.73 m target skeleton the robot could only reach by collapsing into a
        # squat, and even the median 1.66 m AMASS subject ran ~15% under-size.
